...
- **Webhooks salientes + API keys (R40):** subsistema firmado sobre outbox; API keys con scopes. ✅ **Webhooks salientes HECHOS**: `webhooks_salientes` (endpoints con secreto HMAC cifrado, suscripción por tipo o todos) + `entregas_webhook` (registro por intento); listener `EnviarWebhooksSalientes` sobre `EventoDeDominioTenant` firma (HMAC-SHA256, cabecera `X-TurnoUno-Signature`) y entrega; `turnouno:reintentar-webhooks` reintenta las fallidas (agendado c/5 min). CRUD `/webhooks-salientes` (permiso `integraciones.configurar`, secreto devuelto solo al crear). ✅ **API keys con scopes HECHAS**: `llaves_api` tenant-local (solo se guarda el hash sha256 del secreto + un prefijo visible; `scopes` acotan el alcance). CRUD `/llaves-api` (`integraciones.configurar`, secreto devuelto SOLO al crear). Vía de auth paralela para integraciones M2M: middleware `estudio.llave` (resuelve por `X-API-Key`/Bearer contra la BD del tenant, registra último uso) + `alcance:<scope>`; API de lectura `/app/{estudio}/integracion/{miembros,sesiones}` acotada por scope. Aislada por BD (una llave de un estudio no autentica en otro). Scopes de solo lectura por ahora (`CatalogoDePermisosTenant::scopesApi()`). **R40 COMPLETO.**
...
- **Importación CSV (R37):** clientes/membresías/créditos/instructores/productos con preview/errores/rollback. ✅ **Núcleo + miembros HECHO**: `ImportarMiembrosTenant` parsea el CSV (encabezado→filas), valida **fila por fila** (nombre desglosado; correo válido, único en el archivo y contra los existentes) y expone dos fases: **preview** (`POST /importaciones/miembros/preview`, valida SIN escribir, devuelve errores por fila + resumen) e **import** (`POST /importaciones/miembros`, **todo-o-nada**: si alguna fila es inválida no escribe nada = rollback; si todas válidas, crea en una transacción y devuelve el conteo). Tope de 1000 filas; gateado por `miembros.gestionar`. Pendiente: extender el mismo patrón a productos/instructores/membresías/créditos y UI de carga/preview.
